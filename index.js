...
// A real validatin is possible with Regex
function isThisAnEmail2(mail) {
...
function whatDayIsIt() {
    ...
    let days = ['Sunday', 'Monday', 'Tueday', 'Wednesday', 'Thursday', 'Firday', 'Saturday']
...
function howManyDays(aDate) {
    let currentDate = new Date;
    aDate = new Date(aDate);

    let timeDiff = currentDate.getTime() - aDate.getTime()
    let daysPast = timeDiff / (1000 * 3600 * 24)
...
function isTodayMyBirthday(myBirthday) {
    ...
    currentDate = currentDate.toString().substring(4, 10)
    myBirthday = myBirthday.toString().substring(4, 10)
...
function searchAndDivide(str) {
    ...
    for (let i = 0; i < movies.length; i++) {
        ...
        if (movies[i]['Title'].includes(str)) {
            moviesOb.match.push(movies[i])
        }
        else {
            moviesOb.unmatch.push(movies[i])
        }
    }
    return moviesOb;
